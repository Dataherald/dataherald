...
from dataherald.sql_database.models.types import DatabaseConnection, TableSchemaDetail, get_mock_table_schema_detail
...
class TablesSQLDatabaseTool(BaseSQLDatabaseTool, BaseTool):
    ...
    def _run(
        ...
        df = pd.DataFrame(table_representations,columns=['table_name','table_representation'])
        df['table_embedding'] = df.table_representation.apply(lambda x: self.get_embedding(x))
        df['similarities'] = df.table_embedding.apply(lambda x: self.cosine_similarity(x, question_embedding))
        table_relevance = ""
    ...
    async def _arun(
        ...
        raise NotImplementedError("TablesWithRelevanceScoresTool does not support async")
...
class SQLDatabaseToolkit(BaseToolkit):
    ...
    def get_tools(self) -> List[BaseTool]:
        ...
        tables_sql_db_tool = TablesSQLDatabaseTool(db=self.db, context=self.context, db_scan= self.db_scan)
        schema_sql_db_tool = SchemaSQLDatabaseTool(db=self.db, context=self.context, db_scan= self.db_scan)
        info_relevant_tool = InfoRelevantColumns(db=self.db, context=self.context, db_scan= self.db_scan)
        get_fewshot_examples_tool = GetFewShotExamples(db=self.db, context=self.context, few_shot_examples= self.few_shot_examples)
        return [
            query_sql_db_tool,
            tables_sql_db_tool,
            schema_sql_db_tool,
...
class DataheraldSQLAgent(SQLGenerator):
    ...
    **kwargs: Dict[str, Any],
        ...
        prefix = prefix.format(dialect=toolkit.dialect, top_k=top_k, max_examples = max_examples)
    ...
    def generate_response(
        ...
        few_shot_examples = context_store.retrieve_context_for_question(
            user_question,
            number_of_samples=self.max_number_of_examples
            )
        ...
        toolkit = SQLDatabaseToolkit(db=self.database, context=context, few_shot_examples=new_fewshot_examples, db_scan=db_scan)
