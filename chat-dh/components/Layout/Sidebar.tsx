...
export const Sidebar: FC = () => {
  ...
  return (
    <div className="h-full bg-black bg-opacity-5 border-r border-black border-opacity-10 text-primary flex flex-col justify-between px-1 sm:px-2 md:px-4 pb-4">
          ...
          {menuItems.map(({ label, href, icon }, idx) => {
            ...
            return (
              <Link
                key={idx}
                href={href}
                className={`flex items-center p-2 font-bold rounded-lg text-primary ${
                  isActive
      ...
      {user && (
        <div className="flex flex-col gap-2">
          <Link
            ...
            className="flex items-center gap-3 p-2 font-bold my-1 rounded-lg text-primary hover:bg-black hover:bg-opacity-20"
          >
            <Icon value="logout" />
